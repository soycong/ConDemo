...
final class ChatGPTManager {
    ...
    func analyzeTranscript(messages: [MessageData], completion: @escaping (Result<String, Error>) -> Void) async {
...
//                    let analysis = try self.saveAnalysis(messages: messages, response: response)
    ...
    private func requestAnalysis(transcript: String, completion: @escaping (Result<ChatGPTResponse, Error>) -> Void) {
        let text = """
        ...
        let parameters: [String: Any] = [
            ...
            "messages": [
                ["role": "system", "content": "당신은 대화 내용을 분석하고 요약하고, 요약 내용에 대한 poll 3개를 생성하는 도우미입니다. 한국어로 답변해주세요."],
...
//    private func saveAnalysis(messages: [MessageData], response: ChatGPTResponse) throws -> Analysis {
//        guard let content = response.choices.first?.message.content else {
//            throw NSError(domain: String(describing: ChatGPTManager.self), code: 1, userInfo: [NSLocalizedDescriptionKey: "ChatGPT 응답 내용이 없습니다"])
//        }
//        
