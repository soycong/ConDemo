...
struct Transcriber: ParsableCommand {
    ...
    func startTranscriptionJob() async throws {
        guard let bundleFileURL = Bundle.main.url(forResource: "testAudio", withExtension: "flac") else {
        ...
        let transcribeClient = TranscribeClient(config: config)
...
//            let fileURL = URL(fileURLWithPath: path)
        ...
        let startJobRequest = StartTranscriptionJobInput(
            ...
            settings: TranscribeClientTypes.Settings(
                maxSpeakerLabels: 2, showSpeakerLabels: true
            ),
    ...
    func readS3URI(uri: String, region: String = "ap-northeast-2") async throws -> String {
        ...
        if uri.hasPrefix("s3://") {
            ...
            guard let firstSlashIndex = uriWithoutPrefix.firstIndex(of: "/") else {
                throw NSError(domain: "S3URIError", code: 2, userInfo: [NSLocalizedDescriptionKey: "URI 형식이 올바르지 않음"])
            }
            ...
            guard let url = URL(string: uri) else {
                throw NSError(domain: "S3URIError", code: 4, userInfo: [NSLocalizedDescriptionKey: "잘못된 HTTPS URL 형식"])
            }
            
            let pathComponents = url.pathComponents.filter { $0 != "/" }
            guard pathComponents.count >= 2 else {
                throw NSError(domain: "S3URIError", code: 5, userInfo: [NSLocalizedDescriptionKey: "HTTPS URL 경로가 올바르지 않음"])
            }
            
            bucket = pathComponents[0]
            key = pathComponents.dropFirst().joined(separator: "/")
        } else {
            throw NSError(domain: "S3URIError", code: 1, userInfo: [NSLocalizedDescriptionKey: "지원되지 않는 URI 형식입니다. s3:// 또는 https://s3.*.amazonaws.com 형식이어야 합니다."])
        }
        ...
        // GetObject 요청 생성
        let getObjectRequest = AWSS3.GetObjectInput(
            bucket: bucket,
            key: key
        )
        ...
        if let data = try await response.body?.readData(),
           let content = String(data: Data(data), encoding: .utf8) {
            ...
            throw NSError(domain: "S3URIError", code: 3, userInfo: [NSLocalizedDescriptionKey: "데이터를 문자열로 변환할 수 없습니다."])
        }
    }
}
...
extension Transcriber {
    func parseTranscriptionContent(_ content: String) throws -> TranscriptionResult {
        ...
        do {
            let result = try decoder.decode(TranscriptionResult.self, from: jsonData)
            return result
        } catch {
