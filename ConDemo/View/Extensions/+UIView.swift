...
extension UIView {
    ...
    private var keyboardInfo: KeyboardInfo? {
        get {
            objc_getAssociatedObject(self, &UIView.keyboardInfoKey) as? KeyboardInfo
        }
        set {
    ...
    private var observers: [NSObjectProtocol]? {
        get {
            objc_getAssociatedObject(self, &UIView.observersKey) as? [NSObjectProtocol]
        }
        set {
            objc_setAssociatedObject(self,
                                     &UIView.observersKey,
    ...
    func setupKeyboard(bottomConstraint: Constraint,
                       defaultInset: CGFloat,
                       textViews: [UITextView]) {
        removeKeyboard() // 기존 설정 제거

        // 키보드 정보 저장
        keyboardInfo = KeyboardInfo(constraint: bottomConstraint,
        ...
        // 키보드 옵저버 등록
        var newObservers: [NSObjectProtocol] = .init()

        let showObserver = NotificationCenter.default
        ...
        let hideObserver = NotificationCenter.default
    ...
    private func createKeyboardToolbar() -> UIToolbar {
        let toolbar: UIToolbar = .init(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width,
                                                     height: 44))
        toolbar.barStyle = .default

        let flexSpace: UIBarButtonItem = .init(barButtonSystemItem: .flexibleSpace, target: nil,
                                               action: nil)
    ...
    private func keyboardWillShow(notification: Notification) {
        ...
        for textView in info.textViews {
            ...
            if textView.isFirstResponder, let selectedRange = textView.selectedTextRange {
                textView.scrollRectToVisible(textView.caretRect(for: selectedRange.end),
                                             animated: true)
            }
        }
    ...
    private func keyboardWillHide(notification: Notification) {
        guard let info = keyboardInfo else {
            return
        }
